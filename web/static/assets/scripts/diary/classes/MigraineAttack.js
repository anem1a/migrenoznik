class MigraineAttack {
    constructor(local_id, dt_start, strength, login = null, duration = null, triggers = [], symptoms = [], drugs = [], id = null) {
    ...
    add_drug(code) {
        if (!this.has_drug(code)) {
            this.Drugs.push(new MigraineDrug(code));
        }
    }
    ...
    remove_drug(code) {
        this.Drugs = this.Drugs.filter(element => element.Code !== code);
    }
