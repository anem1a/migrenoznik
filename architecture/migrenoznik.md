...
sequenceDiagram
    ...
    participant Локальное Хранилище
    participant Сервер
    Пользователь->>Приложение: Нажать "Удалить"
    Приложение->>Локальное Хранилище: Запросить migraine_attacks
    Локальное Хранилище-->>Приложение: migraine_attacks
    alt Статус == "BACKED_UP"
    Приложение->>Приложение: Присвоить статус "LOCAL_DELETED"
    Приложение->>Сервер: GET api/delete_entry
    alt Сервер вернул ID
    Сервер-->>Приложение: 200 OK
    Приложение->>Приложение: Удалить запись
    else 
    Сервер-->>Приложение: 401/500
    Приложение->>Приложение: Присвоить статус "FAILED_SERVER_DELETING"
    end
    else
    Приложение->>Приложение: Удалить запись
    end
...
Триггер:
- заход на страницу (обновление страницы)
- каждые 15 секунд
...
sequenceDiagram
    ...
    participant Локальное Хранилище
    participant Сервер
    Пользователь->>Приложение: Зайти в приложение
    Приложение->>Приложение: Инициализировать таймер на 15 секунд
    loop Таймер сработал
    Приложение->>Локальное Хранилище: Запросить migraine_attacks
    Локальное Хранилище-->>Приложение: migraine_attacks
    loop Для каждой записи
    alt Статус = "FAILED_SERVER_CREATING"
    Приложение->>Сервер: Отправить запрос на сервер
    Сервер->>Приложение: Ответ
    Приложение->>Приложение: Обработать ответ
    else Статус = "FAILED_SERVER_DELETING"
    Приложение->>Сервер: Отправить запрос на сервер
    Сервер->>Приложение: Ответ
    Приложение->>Приложение: Обработать ответ
    end
